...
class Mega:
    ...
    def _login_user(self, email, password):
        ...
        try:
            ...
            pbkdf2_key = hashlib.pbkdf2_hmac(
                hash_name="sha512", password=password.encode(), salt=a32_to_str(user_salt), iterations=100000, dklen=32
            )
    ...
    def get_files(self):
        ...
        shared_items = {file["h"]: file["ph"] for file in files["ph"]}
        for file in files["f"]:
            # PATCH IN
            file_copy = file.copy()
            processed_file = self._process_file(file, shared_keys)
            if not processed_file["a"]:
                shared_folder_key = processed_file["shared_folder_key"]
                processed_file = self._process_file(file_copy, {"EXP": {}})
                processed_file["shared_folder_key"] = shared_folder_key
            if processed_file["h"] in shared_items:
                processed_file["shared"] = shared_items[processed_file["h"]]
            # PATCH OUT
    ...
    def _download_file(
        self, file_handle, file_key, dest_path=None, dest_filename=None, is_public=False, file=None, no_temp_file=False
    ):
        if file is None:
            if is_public:
                file_key = base64_to_a32(file_key)
            file_data = self._api_request({"a": "g", "g": 1, "p" if is_public else "n": file_handle})
        ...
        with (
            tempfile.NamedTemporaryFile(mode="w+b", prefix="megapy_", delete=False)
            if not no_temp_file
            else open(output_path, "xb")
        ) as temp_output_file:
        ...
        return self._api_request(
            {
                "a": "p",
                "t": dest_node["h"],
                "n": [{"ph": file_handle, "t": 0, "a": encrypted_name, "k": encrypted_key}],
            }
        )
